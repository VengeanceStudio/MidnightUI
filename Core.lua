...
-- Debug command to check state driver counts
SLASH_MUIDEBUG1 = "/muidebug"
SlashCmdList["MUIDEBUG"] = function(msg)
    print("|cff00ff00=== MidnightUI Debug Info ===|r")
    
    -- Get ActionBars module
    local AB = MidnightUI:GetModule("ActionBars", true)
    if AB then
        -- Access the state driver count from ActionBars
        local count = 0
        for k, v in pairs(getfenv(0)) do
            if k == "stateDriverCount" then
                count = v
                break
            end
        end
        print("|cffffcc00ActionBars StateDrivers:|r " .. (count or "Unknown"))
    end
    
    -- Get UnitFrames module
    local UF = MidnightUI:GetModule("UnitFrames", true)
    if UF then
        local count = 0
        for k, v in pairs(getfenv(0)) do
            if k == "ufStateDriverCount" then
                count = v
                break
            end
        end
        print("|cffffcc00UnitFrames StateDrivers:|r " .. (count or "Unknown"))
    end
    
    print("|cff00ff00Current FPS:|r " .. math.floor(GetFramerate()))
    print("|cff00ff00Memory Usage:|r " .. math.floor(collectgarbage("count")) .. " KB")
    print("|cff00ff00Zone:|r " .. (GetZoneText() or "Unknown"))
    print("|cff00ff00In Combat:|r " .. tostring(InCombatLockdown()))
end